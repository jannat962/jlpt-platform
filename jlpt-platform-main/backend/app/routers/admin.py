...
def create_test(test: schemas.MockTestCreate, teacher: models.User = Depends(require_teacher_role), db: Session = Depends(get_db)):
    ...
    # 2. Add questions if provided directly to the relationship list
...
def update_test(test_id: int, test_update: schemas.MockTestCreate, teacher: models.User = Depends(require_teacher_role), db: Session = Depends(get_db)):
    ...
    # Update questions: Clear list. Since cascade="all, delete-orphan" is set on the relationship, 
    # SQLAlchemy will automatically delete the old questions from the database.
    db_test.questions.clear()
    db.flush()
    
    # Add new questions to relationship list
